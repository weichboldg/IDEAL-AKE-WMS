...
| `[RequireVorbauOrPickingOrLeitstandAccess]` | admin, vorbau ODER picking ODER leitstand | FaWorkStepsApiController (`/api/fa-work-steps/toggle-completed`) — Erledigt-Haken aus FA-Abarbeitungsliste (vorbau) UND Leitstand-VK-VA (picking/leitstand) (v1.22.0-Followup) |
...
- **enaio DMS-Quelle = View `vw_IDEAL-AKE_Fertigungsauftraege` (seit v1.22.0)**: Der Sync liest die FA-Zuordnung aus der View `vw_IDEAL-AKE_Fertigungsauftraege` mit `WaNummerTrimmed` als FA-Nummer (frueher: `feld44` fuer WA bzw. `left(feld43,7)` fuer Zeichnung). `EnaioDmsDocument.DocumentType` kann jetzt drei Werte tragen: `Werkstattauftrag`, **neu** `Werkstattauftrag+Zeichnung` (Zeichnung direkt am WA) und ggf. weiterhin `Zeichnung`. **Icon + Sortierung:** Die drei Typen haben je ein eigenes Icon (gerendert vom shared Partial `Views/Shared/_EnaioDmsBadges.cshtml` via `EnaioDmsBadgesViewModel`); `EnaioDmsDocumentRepository.GetByOrderNumbersAsync` sortiert die Links pro FA ZENTRAL nach Typ-Vorrang (Werkstattauftrag+Zeichnung=0, Werkstattauftrag=1, sonst=2, dann nach `EnaioDmsObjectId`), damit alle Views (ProductionOrders/PickingLeitstand/FaWorklist/FaCompletion) automatisch dieselbe Reihenfolge zeigen. Neue enaio-Badge-Darstellungen NICHT inline kopieren — das Partial verwenden (`@await Html.PartialAsync("_EnaioDmsBadges", new EnaioDmsBadgesViewModel(links, showLabel: ...))`).
...
- **IsDone vs IsDonePicking — Lese-Seite (seit v1.21.1)**: `Picking/ToggleDone` schreibt `PickingStatus.IsDonePicking` (Sage-`IsDone` darf nicht beschrieben werden — der Sage-Sync wuerde es ueberschreiben). ALLE Stellen, die "FA erledigt" anzeigen oder filtern (FA-Liste, Leitstand, Picking-Worklist), muessen `IsDone || IsDonePicking` pruefen. Von v1.11 bis v1.21.0 las keine Query das Flag — der Abschliessen-Button war wirkungslos. **Seit v1.22.0** filtern AUCH `FaCompletion/Index` (im `if (!showDone)`-Block) und `FaWorklist/Index` (immer, wie Sage-`IsDone`) auf `IsDone || IsDonePicking` — vorher pruefte der Vorbau nur Sage-`IsDone`, sodass komm-erledigte FAs dort weiterhin auftauchten obwohl sie aus der FA-Liste verschwunden waren. Voraussetzung: `ProductionOrderRepository.GetAllOrderedAsync()` laedt `PickingStatus` per `.Include` (sonst ist `o.PickingStatus` null und das Flag wird nie ausgewertet). **Seit v1.22.0 auch Service-Seite:** Die BomCache-Befuellung laeuft NICHT ueber die EF-Methode `ProductionOrderRepository.GetOpenOrdersInWindowAsync` (die hat **keinen Produktiv-Aufrufer** — nur Interface/Impl/Tests, toter Pfad), sondern ueber die **raw-SQL** `BomCacheSyncService.ReadOpenOrdersInWindowAsync` (eigene `SqlConnection`, `SELECT TOP (@max) ... FROM ProductionOrders po`). Diese raw-SQL UND `FaWorkStepDetectionService` (beide Kandidaten-Queries — `matchedFaCount` UND `candidates`, damit `skipped` stimmt) schliessen jetzt komm-erledigte FAs aus: `IsDone=0 AND NOT EXISTS(SELECT 1 FROM ProductionOrderPickingStatus ps WHERE ps.ProductionOrderId=po.Id AND ps.IsDonePicking=1)`. Vorher belegten komm-erledigte FAs die `Sync:BomCacheMaxOrders`-Cache-Plaetze (nach ProductionDate ASC → aelteste zuerst) und verdraengten echte offene FAs aus dem Cache und damit aus der Artikelinfo. **Fallstrick:** Die raw-SQL ist NICHT mit InMemory testbar — der `bebcca0`-Test prueft nur die ungenutzte EF-Methode und faengt den raw-SQL-Pfad NICHT ab. Bei aenderungen an der Order-Auswahl IMMER beide Stellen pruefen (EF-Methode ist Deko, raw-SQL ist Produktion).
...
- **Leitstand-Spalten VK-VA statisch trotz erweiterbarem Katalog (v1.22.0)**: Der WorkStep-Katalog ist erweiterbar, aber der Leitstand zeigt bewusst NUR die 5 statischen Spalten VK/VL/VE/VT/VA (Filter-Keys cooling/fan/electric/doors/superstructure unveraendert). Neue Katalog-AGs erscheinen NICHT automatisch im Leitstand (YAGNI-Entscheid Spec §8/§11) — wer eine sechste Spalte braucht, muss View + ViewModel + Pivot erweitern.
- **Leitstand VK-VA = Erledigt-Status (IsCompleted), NICHT anwendbar (v1.22.0-Followup)**: Seit dem Followup zeigen/togglen die VK-VA-Haken im `PickingLeitstand/Index` den **Erledigt**-Status (`FaWorkStep.IsCompleted`) — DASSELBE Flag wie die FA-Abarbeitungsliste (`FaWorklist`). Datenquelle ist `IFaWorkStepRepository.GetWorkStepDetailPivotAsync` (orderId -> Code -> `FaWorkStepPivotCell(FaWorkStepId, IsCompleted)`, nur aktive Zeilen, 1000er-Chunking) — NICHT mehr `GetWorkStepPivotAsync` (die alte Code->bool-Methode lebt noch, wird aber nirgends mehr produktiv verwendet). ViewModel: `PickingLeitstandItem.WorkSteps` (Dict Code->Cell) statt der alten 5 Bools HasCooling/HasFan/HasElectric/HasDoors/HasSuperstructure. View: fehlender Code = AG nicht anwendbar = leere Zelle (kein Checkbox). Toggle geht ueber `/api/fa-work-steps/toggle-completed` (Body `{ faWorkStepId, value }`); "anwendbar" wird im Leitstand NICHT mehr gesetzt (nur noch FA-Vervollstaendigung/Auto-Erkennung). VK-VA-Spaltenfilter werden in C# nach dem Pivot-Mapping gefiltert (gerenderter Text "erledigt"/"offen"/"", wie die Datumsspalten — Force-C#-Pagination). **Zugriff:** `/api/fa-work-steps/toggle-completed` traegt jetzt `[RequireVorbauOrPickingOrLeitstandAccess]` (admin/vorbau/picking/leitstand) statt `[RequireVorbauAccess]`, weil sowohl Abarbeitungsliste (vorbau) als auch Leitstand (picking/leitstand) togglen.
...
- **FA-Abarbeitungsliste ist Arbeitsgang-zentriert, NICHT Werkbank-zentriert (seit v1.22.0)**: `FaWorklistController.Index` filtert auf EINEN gewaehlten WorkStep (`?workStepId=`) und zeigt alle offenen FAs mit aktivem (`IsRemoved=0`), nicht-erledigtem `FaWorkStep` dieses AGs — **ueber alle Werkbaenke**. Es gibt genau EINE Erledigt-Spalte (der gewaehlte AG, Ausblende-Logik = dessen `IsCompleted`), die Werkbank ist eine filterbare Info-Spalte (Col-Key `workbench`). Die Vorauswahl ohne `?workStepId` kommt aus `User.DefaultWorkStepId` (nullable FK → `WorkSteps`, Migration 70; im Profil + Benutzerstamm pflegbar, Dropdown via `IWorkStepRepository.GetActiveAsync()`). Das Werkbank-AG-Mapping (`ProductionWorkplaceWorkSteps`) bleibt als Stammdatum erhalten, treibt die Liste aber NICHT mehr — der frueher dokumentierte `mappedSteps.All(f => f.IsCompleted)`-Pfad und das "+N weitere AG"-Orphan-Badge sind entfallen. PrintBom traegt jetzt `[RequirePickingOrVorbauOrFaCompletionAccess]` (read-only BOM auch aus FA-Vervollstaendigung erreichbar).
- **FA-Abarbeitung: zwei kombinierte Default-Filter (Werkbank UND Arbeitsgang, v1.22.0-Followup)**: Neben `User.DefaultWorkStepId` (nullable FK → `WorkSteps`, Migration 70) gibt es jetzt `User.DefaultWorkplaceId` (nullable FK → `ProductionWorkplaces`, **Migration 71** `AddUserDefaultWorkplace`, `OnDelete(SetNull)`, im Profil + Benutzerstamm pflegbar). `FaWorklistController.Index(int? workStepId, int? workplaceId, ...)` faellt fuer beide Parameter auf den jeweiligen User-Default zurueck (wenn der URL-Param null ist). Beide Filter werden **UND-verknuepft** angewandt: gewaehlter WorkStep zwingend, `workplaceId` nur wenn gesetzt (sonst alle Werkbaenke). Die FA-Abarbeitungsliste bleibt arbeitsgang-zentriert — die Werkbank ist ein optionaler Zusatzfilter, kein Pivot. Doku/Beweise: `SQL/71_AddUserDefaultWorkplace.sql` + FreshInstall (Spalte `Users.DefaultWorkplaceId` + FK `FK_Users_ProductionWorkplaces_DefaultWorkplaceId` + Index + History-Insert `20260616070943_AddUserDefaultWorkplace`).
