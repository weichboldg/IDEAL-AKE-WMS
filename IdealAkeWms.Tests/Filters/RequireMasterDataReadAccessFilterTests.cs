...
/// <summary>
/// Tests fuer den neuen Read-Filter aus v1.20.0.
/// admin / masterdata_read / masterdata → Zugriff auf Stammdaten-Listing.
/// Picker etc. werden blockiert.
/// </summary>
public class RequireMasterDataReadAccessFilterTests
{
    ...
    [Fact]
    public async Task HasMasterDataReadAccess_AllowsAccess()
    {
        var currentUserService = new Mock<ICurrentUserService>();
        currentUserService.Setup(x => x.HasMasterDataReadAccessAsync()).ReturnsAsync(true);

        var filter = new RequireMasterDataReadAccessFilter(currentUserService.Object);
    ...
    [Fact]
    public async Task NoMasterDataReadAccess_RedirectsToAccessDenied()
    {
        var currentUserService = new Mock<ICurrentUserService>();
        currentUserService.Setup(x => x.HasMasterDataReadAccessAsync()).ReturnsAsync(false);
